...
export async function readApolloActivity({ client, window, checkedAt, pageSize = DEFAULT_PAGE_SIZE, maxRows = DEFAULT_MAX_ROWS } = {}) {
  ...
  const eventRows = await paginateSelect({
    ...
    selectPage: ({ from, to }) => client.from('apollo_activity_events')
      ...
      .eq('event_type', 'active_interval').gte('occurred_at', queryStart).lt('occurred_at', iso(endMs))
  ...
  const activeCustomers = aggregate.customers.length;
  const averageSecondsPerCustomer = complete && activeCustomers > 0
    ? aggregate.activeSeconds / activeCustomers : null;
  return {
    ...
    activeSeconds: complete ? aggregate.activeSeconds : null,
    activeCustomers,
    averageSecondsPerCustomer,

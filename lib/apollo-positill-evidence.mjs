...
export async function readPositillEvidence({ window, checkedAt = new Date().toISOString(), enabled = false, fetchTopSellers } = {}) {
  if (!enabled) return unavailable(window, checkedAt, 'Positill evidence is disabled until an authenticated existing bridge report is configured and verified for Apollo.');
  const checkedAtMs = Date.parse(checkedAt);
  const checkedAtSast = new Date(checkedAtMs + 2 * 60 * 60 * 1000);
  const todayStart = Number.isFinite(checkedAtMs)
    ? new Date(Date.UTC(checkedAtSast.getUTCFullYear(), checkedAtSast.getUTCMonth(), checkedAtSast.getUTCDate()) - 2 * 60 * 60 * 1000).toISOString()
    : null;
  if (!window || window.kind !== 'day' || window.periodToDate !== true
    || window.start !== todayStart || Date.parse(window.end) !== checkedAtMs) {

...
const TODAY_ARCHIVE_SOURCES = new Set(['nutstore', 'product-manager', 'admin-bulk']);
...
export function isNewImageArchiveSource(value) {
  const source = String(value ?? '').trim();
  return TODAY_ARCHIVE_SOURCES.has(source) || isExcelImageArchiveSource(source);
}
...
export function isNewProductLoaderImageArchiveRow(row, now = new Date()) {
  return isNewImageArchiveSource(row?.archived_by ?? row?.archivedBy)
    && johannesburgArchiveDateKey(row?.archived_at ?? row?.archivedAt)
      === johannesburgArchiveDateKey(now);
}

export function filterProductArchiveSection(rows, section = 'older', now = new Date()) {
  const list = Array.isArray(rows) ? rows : [];
  if (section === 'new-images') {
    return list.filter((row) => isNewProductLoaderImageArchiveRow(row, now));
  }
  if (section === 'older') {
    return list.filter((row) => !isNewProductLoaderImageArchiveRow(row, now));
  }

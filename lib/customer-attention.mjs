...
export function summariseCustomerAttention(rows = [], customers = []) {
  ...
  return {
    ...
    observedWindow: {
      start: validRows.reduce((value, row) => earliest(value, row.started_at), null),
      end: validRows.reduce((value, row) => latest(value, Number.isNaN(new Date(row.last_seen_at).getTime()) ? null : row.last_seen_at), null),
    },

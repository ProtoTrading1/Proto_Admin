 ...
 */
export function buildCampaignApprovalReport(campaigns = [], customers = []) {
  ...
  const attributedEmails = new Set();
  ...
  const enrichedCampaigns = preparedCampaigns.map(({ campaign, index, recipients }) => {
  ...
  const organicApprovals = [...approvedByEmail.values()]
    .filter((customer) => !attributedEmails.has(normalizeEmail(customer.email)))
    .map(approvalEntry)
    .sort((a, b) => (validDate(b.approvedAt) || 0) - (validDate(a.approvedAt) || 0));

  return {
    campaigns: enrichedCampaigns,
    approvalSummary: {
      approvedCustomers: approvedByEmail.size,
      attributedApprovals: attributedEmails.size,
      organicApprovals,
      confirmedOrganic: organicApprovals.filter((customer) => !customer.estimated).length,
      estimatedOrganic: organicApprovals.filter((customer) => customer.estimated).length,
    },
  };
}

export function enrichCampaignsWithApprovals(campaigns = [], customers = []) {
  return buildCampaignApprovalReport(campaigns, customers).campaigns;
}

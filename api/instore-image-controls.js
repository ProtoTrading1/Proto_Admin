...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const [itemResult, controlResult, eventsResult, listingControlResult, listingEventsResult] = await Promise.all([
        ...
        stock.from('instore_listing_controls')
          .select('sku, status, reason, updated_by, updated_at, created_at')
          .eq('sku', sku).maybeSingle(),
        stock.from('instore_listing_control_events')
          .select('id, previous_status, next_status, reason, actor, created_at')
          .eq('sku', sku).order('created_at', { ascending: false }).limit(12),
      ]);
      const error = itemResult.error || controlResult.error || eventsResult.error || listingControlResult.error || listingEventsResult.error;
      ...
      return res.status(200).json({
        ...
        listingControl: listingControlResult.data || { sku, status: 'visible' },
        listingEvents: listingEventsResult.data || [],
      });
    }

    if (req.method !== 'POST') return res.status(405).end();
    const action = String(req.body?.action || '').trim().toLowerCase();
    const isListingAction = action === 'hide-listing' || action === 'restore-listing';
    const status = action === 'hide' || action === 'hide-listing' ? 'hidden' : action === 'restore' || action === 'restore-listing' ? 'visible' : '';
    if (!status) return res.status(400).json({ error: 'action must hide, restore, hide-listing or restore-listing.' });
    const reason = text(req.body?.reason);
    if (status === 'hidden' && !reason) return res.status(400).json({ error: 'A reason is required before hiding an Instore image or listing.' });
    ...
    const { data: control, error } = isListingAction
      ? await stock.rpc('set_instore_listing_control', { p_sku: sku, p_status: status, p_reason: reason || null, p_actor: actor })
      : await stock.rpc('set_instore_image_control', {
        p_sku: sku,
        p_status: status,
    ...
    if (error) throw error;
    return res.status(200).json({ ok: true, kind: isListingAction ? 'listing' : 'image', item: { sku, image_url: item.image_url || null }, control });
  } catch (error) {
